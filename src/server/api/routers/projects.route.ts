...
import { createProject, projectValidation, IdValidation, imageWithIdValidation } from "~/validations";
...
export const projectRouter = createTRPCRouter({
   createProject: protectedProcedure.input(createProject).mutation(async ({ ctx, input }) => {
      try {
         ...
         const project = await ctx.db
            ...
            .returning();
         if (!project?.[0])
            throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Unable to create new project" });
   ...
   deleteProject: protectedProcedure.input(IdValidation).mutation(async ({ ctx, input }) => {
      try {
         const project = await ctx.db
            ...
            .returning();
         if (!project?.length) throw new TRPCError({ code: "NOT_FOUND", message: "Project not found" });
   ...
   addImage: protectedProcedure.input(imageWithIdValidation).mutation(async ({ ctx, input }) => {
      try {
         const { id, publicUrl, secureUrl } = input;
         const project = await ctx.db
            .update(projects)
            .set({ publicUrl, secureUrl })
            .where(and(eq(projects.id, id), eq(projects.userId, ctx.user.id)))
            .returning();
         if (!project?.[0]) throw new TRPCError({ code: "NOT_FOUND", message: "Project not found" });
   ...
   removeImage: protectedProcedure.input(IdValidation).mutation(async ({ ctx, input }) => {
      try {
         const { id } = input;
         const project = await ctx.db
            .update(projects)
